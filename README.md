...
2. A rotation matrix representing the direction of the camera's frustum (mimicking a human eye) can be transposed and used to unwrap vertices of objects in world space into view space.
  * Now the vertices of objects exist in the space of what the camera can see.
  * Yes, in every 3D game you've ever played, you are not moving through the world, but the world is moving around you in reverse.
...
6. What can be seen depends on a near and far clip plane to "clip" vertices out that are behind the camera and too far away to see.
  * You can see this in this project by moving close to the car and seeing the edges disappear.
7. By using quaternions instead of Euler angles, the camera avoids loss of degree of freedom (Gimbal Lock).
  * This happens when the pitch is rotated 90 degrees and looks up at the sky.
  * Yaw depends on pitch and roll depends on yaw to calculate the up and forward axes.
